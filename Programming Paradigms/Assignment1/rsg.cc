...
// Recursively generates random text
void rec(map<string, Definition>& grammar, string non_terminal, vector<string>& terminals) {
  Production curr_production = grammar[non_terminal].getRandomProduction();
  Production::iterator it_curr = curr_production.begin();
  Production::iterator it_end = curr_production.end();
  while (it_curr != it_end) {
    string curr_str = *it_curr;
    if (!curr_str.empty() && curr_str[0] == '<' && curr_str[curr_str.size() - 1] == '>') {
      rec(grammar, curr_str, terminals);
    } else {
...
// Checks if ch is a punctiation symbol
bool isPunct(string ch) {
  return ch == "!" || ch == "." || ch == "," || ch == ":" || ch == ";";
}

// Prints the given terminals
void print(vector<string>& terminals, int version) {
