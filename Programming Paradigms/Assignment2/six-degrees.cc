...
void generateShortestPath(const string& source,const string& target, const imdb& db) {
  ...
  while (!partialPaths.empty() && partialPaths.front().getLength() <= 5) {
    ...
    db.getCredits(frontPath.getLastPlayer(), lastActorMovies);
    for (int i = 0; i < (int)lastActorMovies.size(); ++i) {
      film movie = lastActorMovies[i];
      if (seenFilms.find(movie) == seenFilms.end()) {
        seenFilms.insert(movie);
        vector<string> actorsInThisMovie;
        db.getCast(movie, actorsInThisMovie);
        for (int j = 0; j < (int)actorsInThisMovie.size(); ++j) {
          string actor = actorsInThisMovie[j];
          if (seenActors.find(actor) == seenActors.end()) {
            seenActors.insert(actor);
            path newPath = frontPath;
            newPath.addConnection(movie, actor);
            if (actor == target) {
              cout << endl << newPath << endl << endl;
              return;
            }
