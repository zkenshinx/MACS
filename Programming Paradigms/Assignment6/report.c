...
{ 
  pthread_mutex_lock(&bank->transfer_lock);
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&bank->transfer_lock);
...
{
  pthread_mutex_lock(&bank->do_report_lock);
  bank->still_working--;
  if (bank->still_working == 0) {
    pthread_cond_broadcast(&bank->cond);
    bank->still_working = numWorkers;
  } else {
    pthread_cond_wait(&bank->cond, &bank->do_report_lock);
    pthread_mutex_unlock(&bank->do_report_lock);
    return 0;
  }
