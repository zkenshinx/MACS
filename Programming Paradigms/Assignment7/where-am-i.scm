...
(define (partition pivot num-list comp-fn)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) comp-fn)))
	(if (comp-fn (car num-list) pivot)
...
(define (quicksort num-list comp-fn)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) comp-fn)))
	(append (quicksort (car split) comp-fn) 
		(list (car num-list)) 
		(quicksort (cadr split) comp-fn)))))
...
;;
(define (same-pt pt1 pt2)
  (and (= (x pt1) (x pt2)) (= (y pt1) (y pt2)))
)

(define (intersection-points circles-lst)
	(if (< (length circles-lst) 2) '()
    (append
      (apply append 
        (map (lambda (elem) (intersect (car circles-lst) elem)) (cdr circles-lst))
      )
      (intersection-points (cdr circles-lst))
    )
  )
)


(define (distance-product init-pt points-lst)
  (apply *
    (map (lambda (pt) 
          (if (same-pt init-pt pt)
            1 (dist init-pt pt)         
          )
         )
      points-lst
...
(define (rate-points points-lst)
  (map 
    (lambda (pt) 
      (list (distance-product pt points-lst) pt)
    )
  points-lst
  )
)

(define (sort-points rate-points-lst)
  (define (rate-points-comp-fn a b)
      (< (car a) (car b)))
  (quicksort rate-points-lst rate-points-comp-fn)
)

(define (clumped-points points-lst)
  (map (lambda (elem) (cadr elem))
    (prefix-of-list (sort-points (rate-points points-lst)) (quotient (length points-lst) 2))
  )
)

(define (average-point points-lst)
  (let ((avg-point
        (map (lambda (el) (/ el (length points-lst)))
        (apply map (cons + points-lst)))))
    (cons (distance-product avg-point points-lst)
      (list avg-point)
    )
  )
)

(define (best-estimate circles-lst)
  (average-point (clumped-points (intersection-points circles-lst)))
)

(define (where-am-i distances stars)
  (sort-points
    (map (lambda (elem) (best-estimate elem))
